...
# In[33]:
df = pd.read_csv("../../data/data_matrix_without_img_20k.csv")
df = df.drop(df.columns[0],axis=1)
#%%
CLASS = pd.read_csv("../../data/Class_20k.csv")
#%%
...
le.fit(CLASS.values.ravel()) #flattens the dimension
labels = le.transform(CLASS.values.ravel())
...
one = df1.loc[df1[df1.shape[1]-1] == 0] #Seggregate the data
two = df1.loc[df1[df1.shape[1]-1] == 1]
three = df1.loc[df1[df1.shape[1]-1] == 2]
four = df1.loc[df1[df1.shape[1]-1] == 3]
...
#splitting x and y
train_x = train[train.columns[train.columns!=df1.shape[1]-1] ]
print(train_x.shape)
train_y = train.iloc[:,-1]
print(train_y.shape)

valid_x = valid[valid.columns[valid.columns!=df1.shape[1]-1] ]
print(valid_x.shape)
valid_y = valid.iloc[:,-1]
print(valid_y.shape)

test_x = test[test.columns[test.columns!=df1.shape[1]-1] ]
print(test_x.shape)
test_y = test.iloc[:,-1]
print(test_y.shape)
